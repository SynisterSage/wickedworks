...
const ShopPage: React.FC<{ onViewProduct: (handle: string) => void, onToggleSave: (handle: string) => void, savedHandles: string[], onNavigate: (view: ViewMode) => void }> = ({ onViewProduct, onToggleSave, savedHandles, onNavigate }) => {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return allProducts.filter(product => {
      const searchMatch = !lowercasedQuery || product.title.toLowerCase().includes(lowercasedQuery);
  ...
  }, [allProducts, searchQuery, selectedCategories, selectedSizes, selectedColors]);
  ...
  const FilterGroup = ({ title, options, current, setter }: { title: string, options: string[], current: string[], setter: (l: string[]) => void }) => (
    <div className="mb-12">
      ...
      <div className="space-y-3.5">
        {options.map(opt => {
          ...
          return (
            <button
              key={opt}
              onClick={() => toggleFilter(current, setter, opt)}
  ...
  return (
    <div className="pt-32 pb-24 min-h-screen bg-bg-primary">
      {/* Container padding exactly matches Header.view.tsx to ensure perfect vertical alignment */}
      <div className="container mx-auto px-6 md:px-12 lg:px-16 xl:px-20">
        {/* Unified Technical Breadcrumbs */}
        <nav className="flex items-center gap-3 mb-16 overflow-x-auto whitespace-nowrap no-scrollbar border-b border-border-color pb-4">
          ...
          <div className="ml-auto flex items-center gap-4 pl-6">
            <span className="text-[9px] font-mono text-text-primary/10 uppercase tracking-widest hidden md:block italic">ACTIVE_MODULES: {filteredProducts.length.toString().padStart(2, '0')}</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 xl:gap-24">
            ...
            <div className="flex justify-between items-center mb-12">
              ...
              {(selectedCategories.length > 0 || selectedSizes.length > 0 || selectedColors.length > 0) && (
                <button 
                  ...
                  className="text-[8px] font-black uppercase tracking-widest text-neonRed hover:text-text-primary transition-colors"
                >
                  RESET_ALL
                </button>
              )}
          ...
          <main className="lg:col-span-9">
            <header className="mb-16">
               {/* Mobile/Tablet Header: Inline Title & Icon Button */}
               <div className="flex items-center justify-between mb-8 lg:hidden">
                  <div className="flex items-center gap-4">
                      <div className="h-8 w-1.5 bg-neonRed shadow-neon"></div>
                      <h2 className="text-4xl font-black uppercase tracking-tighter text-text-primary italic leading-none">Shop_All</h2>
                  </div>
                  
                  <button 
                    onClick={() => setIsFilterOpen(true)}
                    className="w-12 h-12 flex items-center justify-center bg-bg-secondary border border-border-color text-text-primary hover:text-neonRed hover:border-neonRed transition-all shadow-lg active:scale-95 group"
               ...
               <div className="hidden lg:flex items-center gap-6 mb-10">
                  ...
                  <h2 className="text-7xl xl:text-8xl font-black uppercase tracking-tighter text-text-primary leading-none italic">
                    ACTIVE <span className="text-neonRed drop-shadow-[0_0_30px_rgba(255,0,60,0.3)]">INVENTORY.</span>
                  </h2>
               </div>
            </header>

            <div className="relative mb-16 group">
              <div className="absolute left-6 top-1/2 -translate-y-1/2 text-text-secondary/20 group-focus-within:text-neonRed transition-colors pointer-events-none">
                <Icons.Search />
              </div>
              <input 
                type="text"
                ...
                className="w-full bg-bg-secondary border border-border-color text-xl font-black uppercase tracking-tighter text-text-primary focus:outline-none focus:border-neonRed/30 transition-colors py-6 pl-16 pr-16 placeholder:text-text-primary/10"
              />
              {searchQuery && (
                <button 
                  onClick={() => setSearchQuery('')}
                  className="absolute right-6 top-1/2 -translate-y-1/2 text-text-secondary/40 hover:text-neonRed transition-colors"
            ...
            {filteredProducts.length > 0 ? (
              ...
              <div className="h-[60vh] flex flex-col items-center justify-center text-center bg-bg-contrast-01 border border-dashed border-border-color p-12">
                ...
                <button 
                  onClick={clearFiltersAndSearch}
                  className="px-10 py-5 bg-neonRed text-white font-black uppercase tracking-[0.4em] text-[10px] shadow-neon hover:shadow-neon-strong transition-all active:scale-95"
                >
                  Clear Selection
                </button>
              </div>
