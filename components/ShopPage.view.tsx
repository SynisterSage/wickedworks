...
interface ShopPageViewProps {
  ...
  sort: 'new' | 'best' | 'featured' | 'price-asc' | 'price-desc' | undefined;
  onSortChange: (value: 'new' | 'best' | 'featured' | 'price-asc' | 'price-desc' | undefined) => void;
...
export const ShopPageView: React.FC<ShopPageViewProps> = ({
  ...
  const FilterGroup = ({ title, options, current, type, scrollable }: { title: string, options: string[], current: string[], type: 'categories' | 'sizes' | 'colors', scrollable?: boolean }) => (
    <div className="mb-12">
      ...
      <div className={`space-y-3.5 ${scrollable ? 'max-h-64 overflow-y-auto pr-2 custom-scrollbar' : ''}`}>
        {options.map(opt => {
  ...
  const hasActiveFilters = selectedFilters.categories.length > 0 || selectedFilters.sizes.length > 0 || selectedFilters.colors.length > 0 || selectedFilters.priceMin !== '' || selectedFilters.priceMax !== '';

  const SortSelect = () => (
    <div className="flex items-center gap-3">
      <span className="text-[9px] font-black uppercase tracking-[0.3em] text-text-primary/40">Sort</span>
      <select
        className="bg-bg-secondary border border-border-color text-[11px] font-black uppercase tracking-[0.2em] text-text-primary px-3 py-2 focus:outline-none focus:border-neonRed/50"
        value={sort || ''}
        onChange={(e) => onSortChange(e.target.value ? (e.target.value as typeof sort) : undefined)}
      >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 xl:gap-24">
          ...
          <main className="lg:col-span-9">
            ...
            <div className="flex items-center justify-between mb-10 lg:hidden">
              <SortSelect />
              {filterOptions.price && (
                <div className="text-[9px] text-text-primary/40 uppercase tracking-[0.3em]">{filterOptions.price.min.toFixed(0)} - {filterOptions.price.max.toFixed(0)}</div>
              )}
            </div>
